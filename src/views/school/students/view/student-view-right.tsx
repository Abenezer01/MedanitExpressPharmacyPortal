...
import StudentsAttendanceTab from "./tabs/students-attendance-tab";
...
const StudentViewRight = ({ tab, student, isLoading }: Props) => {
  ...
  const handleChange = (event: SyntheticEvent, value: string) => {
    ...
    router
      .push({
        pathname: `/school/students/${student?.id}/${value.toLowerCase()}`,
      })
      .then(() => {
        event;
      });
        ...
        {isLoading ? (
          ...
          <>
            <TabPanel sx={{ p: 0 }} value="profile">
              <MemberAboutOverview member={student?.member} showSmallTeamAttendance={false} />
            </TabPanel>
            <TabPanel sx={{ p: 0 }} value="attendance">
              <StudentsAttendanceTab student={student} />
            </TabPanel>
            <TabPanel sx={{ p: 0 }} value="grades">
              {/* <GradesTab student={student} /> */}
            </TabPanel>
            <TabPanel sx={{ p: 0 }} value="family">
              {/* <SupportFamilyTab student={student} /> */}
            </TabPanel>
            <TabPanel sx={{ p: 0 }} value="gallery">
              {/* <GalleryTab student={student} /> */}
            </TabPanel>
          </>
        )}
      </Box>
